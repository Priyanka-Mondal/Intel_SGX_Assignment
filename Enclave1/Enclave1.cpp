...
//CREATE SESSION
uint32_t createSession(sgx_enclave_id_t src_enclave_id,
...
//SEND MESSAGE
uint32_t send_message(sgx_enclave_id_t src_enclave_id,
                               sgx_enclave_id_t dest_enclave_id, uint32_t secret_data)
{
    ATTESTATION_STATUS ke_status = SUCCESS;
    uint32_t target_fn_id, msg_type;
    char* marshalled_inp_buff;  // char* marshalled_inp_buff;
    ...
    ke_status = my_marshal_message_exchange_request(target_fn_id, msg_type, secret_data, &marshalled_inp_buff, &marshalled_inp_buff_len); // &str or str
    if(ke_status != SUCCESS)
    {
        return ke_status;
    }
...
//CLOSE SESSION
uint32_t closeSession(sgx_enclave_id_t src_enclave_id,
                                sgx_enclave_id_t dest_enclave_id)
{
    dh_session_t dest_session_info;
    ATTESTATION_STATUS ke_status = SUCCESS;
